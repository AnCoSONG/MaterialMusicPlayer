...
bool FLACcover::verificationPictureFormat(char *data)
{
...
//安全释放内存
void FLACcover::freePictureData()
{
    if (pPicData)
    {
        delete pPicData;
...
bool FLACcover::loadPictureData(const char *inFilePath)
{
    freePictureData();
    FILE *fp = NULL;
...
//取得图片数据的长度
int FLACcover::getPictureLength()
{
    return picLength;
}

//取得指向图片数据的指针
byte *FLACcover::getPictureDataPtr()
{
    return pPicData;
}

//取得图片数据的扩展名（指针）
char *FLACcover::getPictureFormat()
{
    return picFormat;
}

bool FLACcover::writeCover(char *filename){
    FILE *fp = NULL;
    std::string name;
...
bool FLACcover::writePictureDataToFile(const char *outFilePath)
{
    FILE *fp = NULL;
    if (picLength > 0)
    {
...
bool FLACcover::extractPicture(const char *inFilePath, const char *outFilePath)
{
...
bool FLACcover::readCover(QString inPath, const char * outName){
    QTextCodec *code = QTextCodec::codecForName("gb18030");
    std::string ts = code->fromUnicode(inPath).data(); //utf8转gdk gb2312
    const char *path = ts.c_str();
    if (loadPictureData(path))	//如果取得图片数据成功
    {
        qDebug()<<"读取成功";
