...
bool MP3cover::verificationPictureFormat(char *data)
{
...
bool MP3cover::loadPictureData(const char *inFilePath)
{
    freePictureData();
...
//取得图片数据的长度
int MP3cover::getPictureLength()
{
    return picLength;
}

//取得指向图片数据的指针
byte *MP3cover::getPictureDataPtr()
{
    return pPicData;
}

//取得图片数据的扩展名（指针）
char *MP3cover::getPictureFormat()
{
    return picFormat;
}

bool MP3cover::writePictureDataToFile(const char *outFilePath)
{
    FILE *fp = NULL;
    if (picLength > 0)
    {
...
bool MP3cover::extractPicture(const char *inFilePath, const char *outFilePath)
{
...
bool MP3cover::writeCover(char *filename)
{
    FILE *fp = NULL;
    std::string name;
...
bool MP3cover::readCover(QString inPath, const char *outName)
{
